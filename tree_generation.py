...
#-----------------------------------------------    
def tree_generation5(dV_max):
    
    from loweringmaneuver_test import lowering_maneuver
    from raisingmaneuver_test import raising_maneuver
    from distance_SSP_to_target import distance_SSP_to_target
    import math
    import numpy as np
    from propagate_vectorize import propagate
    ...
    for i in range(num_sats):
        ...
        sat_epoch = datetime.datetime(2010,10,10,12)


    i =0
    prop_sec = 3600*24*2.5; 
    for sat in satellites:
        new_sat = propagate(mu, Re, J2, vel_e, rad, flattening, sat, prop_sec)
        #new_sat[10]=0
        satellites[i] = new_sat
        i=i+1;
    
    #this is the max acceleration available on the satellite
    accel = 0.35*10**(-3)/4*10000 #m/s^2
...
#    targets = [[targets_time[i], targets_lat[i], targets_long[i]] for i in range(num_targets)]
    ...
    for target in targets:
        ...
        #print('---')
        #print("Exploring options for this target"+"\t")
        #print(str((current_target)) +"\n")
        ...
        if solution_found == False:
            ...
            for origin in calc_nodes_last_target: 
                ...
                for x in range(len(old_sat_list)):
                    ...
                    propagate_sat =[]
                    calc_nodes_current_target.append(current_node)
                    G.add_edge(origin, current_node, target_accesses = 0, dV = 99, dist_to_target = distance, man_type = flag)
                    current_node = current_node+1           
    ...
    w = csv.writer(open("output_tree_5_5_2021_maxdelta_v.csv", "w"))
    ...
    nx.draw(G, pos, node_size=50, alpha=0.5, node_color="white", with_labels=True)
...
#    
    ...
    plt.axis('equal')
    plt.savefig('test_tree_5_5.png')
#    
#    
    
    return [dV_total, -num_accesses_total, dist_mean, G, nodes]

[dV, accesses, dist, G, nodes] = tree_generation5(100)
